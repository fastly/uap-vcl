    ...
    it(`receives a response for ${test.user_agent_string} to be ${test.family} / ${test.major}`, () =>
      chai.request(uri)
        ...
        .then(res => {
          ...
          expect(res).to.be.html;
          expect(res.text).to.include('ua-parser in VCL');
          expect(res.text).to.include(`<th>ua_family</th><td>${test.family}</td>`);
          expect(res.text).to.include(`<th>ua_major</th><td>${test.major}</td>`);
          expect(res.text).to.include(`<th>ua_minor</th><td>${test.minor}</td>`);
          expect(res.text).to.include(`<th>ua_patch</th><td>${test.patch}</td>`);
        })
    ).timeout(5000);
  });
} catch (e) {
